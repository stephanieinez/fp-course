...
bindParser ::
  ...
  -> Parser a
  -> Parser b
-- bindParser f p = P (\i -> case parse p i of Result j a -> parse (f a) j)
...
-- OR :: lift2 (:.) <*> list -> teach :. about parsers (lift2) then add one or many
--
-- p and then call it x (0 or many) p and then call it y always (x:.y)
...
satisfy ::
  (Char -> Bool)
  -> Parser Char
satisfy c2b = character >>= (\c -> if c2b c then pure c else unexpectedCharParser c)
-- OR :: c2b -> character >>= \c -> bool (unexpectedCharParser c) (pure c) (c2b c)

  -- true = pure c
  -- false = unexpectedCharParser
-- satisfy f = bindParser (if then else unexpectedCharParser ) character
...
-- lift3 (\ u f r ->  u:. f ++ r) upper (thisMany 5 lower) (list lower)
